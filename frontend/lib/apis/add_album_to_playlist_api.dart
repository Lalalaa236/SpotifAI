...
class AddAlbumToPlaylistApi {
  ...
  static Future<dynamic> createPlaylistAndAddAlbum(String playlistName, int albumId) async {
    try {
      await _setCsrfTokenAndCookies();

      final createPlaylistResponse = await DioClient.instance.post('/v1/playlists/', data: {
        'name': playlistName,
      });

      final playlistId = createPlaylistResponse.data['id'];

      return await DioClient.instance.post('/v1/playlists/$playlistId/add_album_to_playlist/', data: {
        'album_id': albumId,
      });
    } 
    catch (e) {
  ...
  static Future<dynamic> addAlbumToExistingPlaylist(int playlistId, int albumId) async {
    try {
      await _setCsrfTokenAndCookies();
